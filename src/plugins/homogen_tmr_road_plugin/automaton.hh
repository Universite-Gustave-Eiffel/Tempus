...
namespace Tempus {
    ...
    public:	
		...
		class PenaltyCalculator {
			public:
				PenaltyCalculator( int mode ) : mode_(mode) {} // constructeur
	
				double operator() ( Graph& graph, Vertex& v ) {
					for (std::map<int, double >::iterator penaltyIt = graph[v].penaltyPerMode.begin() ; penaltyIt != graph[v].penaltyPerMode.end() ; penaltyIt++ )
						{
						if (mode_ && penaltyIt->first) return penaltyIt->second; 
						}
					return 0; 
				} 
			private:
				unsigned int mode_; // stockage du mode
		}; 
		...
		void build_graph( Road::Restrictions sequences )
		{
			...
			for ( std::list<Road::Restriction>::const_iterator it=sequences.restrictions().begin() ; it!=sequences.restrictions().end() ; it++ ) {
		...
		void add_sequence( std::vector< Entity > entity_sequence, std::map< int, double > penaltyPerMode ) // corresponds to adding a sequence in the "goto" function
		{
			...
			{
				std::pair< Vertex, bool > transition_result = find_transition( automaton_graph_, current_state_, entity_sequence[j] ); 
				...
				else // this state needs to be inserted
				{
					...
					if ( j == entity_sequence.size()-1 ) automaton_graph_[ s ].penaltyPerMode = penaltyPerMode; 
					current_state_ = s; 
				} 
			}
		} 
		...
		std::map < Vertex, Vertex > build_failure_function()
		{
			...
			BGL_FORALL_OUTEDGES_T( initial_state_, edge, automaton_graph_, Graph ) {
				...
				failure_function_.insert( std::pair< Vertex, Vertex > ( target( edge, automaton_graph_ ), initial_state_) ) ; 
			} 
			...
			while ( ! q.empty() )
			{
				...
				BGL_FORALL_OUTEDGES_T( s, edge, automaton_graph_, Graph ) {
					...
					while ( find_transition( automaton_graph_, r, automaton_graph_[edge].entity ).second == false && r != initial_state_ ) r = failure_function_[ r ];
					if ( find_transition( automaton_graph_, r, automaton_graph_[edge].entity ).second == true ) 
						failure_function_.insert( std::pair< Vertex, Vertex > ( s, find_transition(automaton_graph_, r, automaton_graph_[edge].entity).first ) ) ; 
					else failure_function_.insert( std::pair< Vertex, Vertex > ( s, initial_state_ ) ) ; 
				} 
			}
		...
		void build_shortcuts(std::map < Vertex, Vertex > failure_function_) // corresponds to building the "next" function
		{
			std::list< Vertex > q; 
			...
			while ( ! q.empty() )
			{
				...
				if ( failure_function_[ r ] != initial_state_ ) 
				{
					BGL_FORALL_OUTEDGES_T( failure_function_[ r ], edge, automaton_graph_, Graph ) {	
						Edge e; 
						bool ok; 
	...
	template <class Entity>
	std::pair< typename Automaton<Entity>::Vertex, bool > find_transition( const typename Automaton<Entity>::Graph& automaton_graph_,
									   typename Automaton<Entity>::Vertex q, 
									   Entity e)
	{
		BGL_FORALL_OUTEDGES_T( q, edge, automaton_graph_, typename Automaton<Entity>::Graph ) {
			if ( automaton_graph_[ edge ].entity == e ) {
				typename Automaton<Entity>::Vertex r=target(edge, automaton_graph_);
